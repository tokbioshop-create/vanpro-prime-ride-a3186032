...
export function DepoimentosCarrossel() {
  ...
  const itens = [
    ...lista.map((f) => ({ nome: f.nome, mensagem: f.mensagem, estrelas: f.estrelas })),
    ...padrao,
  ].slice(0, 10);
  ...
  return (
    <section className="mt-4">
      <div className="mb-2 flex items-baseline justify-between px-1">
        ...
        <span className="text-[11px] text-muted-foreground">Atualizado automaticamente</span>
      </div>
      ...
      <div className="-mx-1 overflow-hidden px-1 pb-2">
        <div className="animate-marquee-x flex w-max gap-3">
          {loop.map((f, i) => (
            <article
              key={i}
              className="bg-navy w-[70vw] max-w-[280px] shrink-0 rounded-2xl p-4 shadow-[var(--shadow-brand-soft)] ring-1 ring-[oklch(1_0_0/0.1)]"
            >
              ...
              <p className="mt-2 line-clamp-3 text-[12px] leading-relaxed text-[oklch(0.88_0.02_265)]">
                {f.mensagem}
              </p>
            </article>
          ))}
