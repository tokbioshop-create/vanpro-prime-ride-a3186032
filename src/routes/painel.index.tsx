  ...
  head: () => ({
    meta: [
      { title: "Painel da empresa — VanPro" },
      {
        name: "description",
        content:
          "Painel de controle do empresário VanPro: dados da empresa, agendamentos, QR Code, reservas, recebimentos e contatos.",
      },
      { property: "og:title", content: "Painel da empresa — VanPro" },
      {
        property: "og:description",
        content: "Configure tudo o que seus clientes veem no aplicativo VanPro.",
      },
    ],
  }),
  component: Painel,
...
const secoes = [
  {
    ...
    hint: "Nome, cidade, CNPJ e descrição",
  },
  {
  ...
  return (
    <AppScreen title="Painel da empresa" subtitle={config.empresa.nome}>
      <div className="card-elevated flex items-center gap-4 p-4">
        <span className="bg-brand flex size-14 items-center justify-center rounded-2xl text-lg font-bold text-primary-foreground">
          {config.empresa.sigla}
        </span>
      ...
      <div className="space-y-3">
        {secoes.map(({ to, icon: Icon, label, hint }) => (
          <Link
            key={to}
            to={to}
            className="press bg-brand flex items-center gap-3.5 rounded-2xl p-4 text-primary-foreground shadow-[var(--shadow-brand-soft)]"
          >
            <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[oklch(1_0_0/0.18)]">
              <Icon className="size-5" strokeWidth={2.2} />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-bold">{label}</span>
              <span className="block text-[11px] text-[oklch(1_0_0/0.75)]">{hint}</span>
            </span>
