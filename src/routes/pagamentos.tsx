...
function PixLogo({ className = "size-8" }: { className?: string }) {
  return <img src="/pix-logo.svg" className={className} alt="Pix" />;
}
function DebitLogo({ className = "size-9" }: { className?: string }) {
  return <img src="/debit-card.svg" className={className} alt="Cartão de débito" />;
}
...
function Pagamentos() {
  ...
  return <div className="min-h-screen bg-[#f5f6f8] text-[#071342]"><div className="mx-auto min-h-screen w-full max-w-md bg-white pb-8 shadow-[0_0_35px_rgba(15,23,42,0.05)]">
    <header className="flex h-[64px] items-center border-b border-slate-100 bg-white px-4">
      <button onClick={() => navigate({ to: "/carrinho" })} aria-label="Voltar ao carrinho" className="flex size-9 items-center justify-center rounded-full"><ArrowLeft className="size-[19px]"/></button>
      <div className="ml-2 flex flex-1 items-center gap-2.5"><div className="flex size-8 items-center justify-center rounded-lg bg-[#071342] text-sm font-black text-white">V</div><div><p className="text-[14px] font-black leading-none">Van<span className="text-[#f4a900]">Mox</span></p><p className="mt-0.5 text-[9px] font-medium uppercase tracking-[0.12em] text-slate-400">Pagamento seguro</p></div></div><LockKeyhole className="size-[17px] text-slate-400"/>
    </header>

    <main>
      <img src="/checkout-banner.svg" alt="Sua viagem, seu conforto. Finalize seu pagamento com segurança." className="block h-[170px] w-full object-cover" />
