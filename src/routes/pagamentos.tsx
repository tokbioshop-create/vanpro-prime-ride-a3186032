...
function PixLogo({ className = "size-8" }: { className?: string }) {
  return <svg viewBox="0 0 48 48" className={className} aria-label="Pix"><g fill="#32BCAD"><path d="M24 4.5c2.7 0 5.3 1.1 7.2 3l9.3 9.3a5.9 5.9 0 0 1 0 8.4l-9.3 9.3a10.2 10.2 0 0 1-14.4 0l-9.3-9.3a5.9 5.9 0 0 1 0-8.4l9.3-9.3a10.2 10.2 0 0 1 7.2-3Z"/><path fill="white" d="m16.1 17 3.9-3.9a5.6 5.6 0 0 1 8 0l3.8 3.8-2.9 2.9-3.5-3.5a2 2 0 0 0-2.8 0l-3.6 3.6-2.9-2.9Zm15.8 14-3.9 3.9a5.6 5.6 0 0 1-8 0l-3.8-3.8 2.9-2.9 3.5 3.5a2 2 0 0 0 2.8 0l3.6-3.6 2.9 2.9Z"/></g></svg>;
}

function VisaLogo() { return <span aria-label="Visa" className="text-[18px] font-black italic tracking-[-0.08em] text-[#1434CB]">VISA</span>; }
...
function Pagamentos() {
  ...
  if (pago) return <div className="min-h-screen bg-white px-5 py-8 text-[#071342]"><div className="mx-auto flex min-h-[80vh] max-w-md flex-col items-center justify-center text-center"><div className="flex size-16 items-center justify-center rounded-full bg-emerald-50 ring-1 ring-emerald-100"><CheckCircle2 className="size-9 text-emerald-500"/></div><p className="mt-5 text-xs font-black uppercase tracking-[0.16em] text-[#0b2be3]">VanMox</p><h1 className="mt-2 text-2xl font-extrabold">Reserva confirmada</h1><p className="mt-2 text-sm text-slate-500">Pagamento de {brl(valor)} aprovado com sucesso.</p><button onClick={() => navigate({ to: "/reservas" })} className="press mt-7 h-13 w-full rounded-xl bg-[#0b2be3] font-extrabold text-white shadow-[0_8px_22px_rgba(11,43,227,0.16)]">Ver minhas reservas</button></div></div>;

  const optionClass = (selected: boolean) => `press flex min-h-[62px] w-full items-center gap-3 rounded-xl border bg-white px-4 text-left transition ${selected ? "border-[#159b86] bg-[#fbfefd] ring-1 ring-[#159b86]/25" : "border-slate-200"}`;

  return <div className="min-h-screen bg-[#f7f8fa] text-[#071342]"><div className="mx-auto min-h-screen w-full max-w-md bg-white pb-8 shadow-[0_0_35px_rgba(15,23,42,0.04)]">
    <header className="flex h-[68px] items-center border-b border-slate-100 bg-white px-4">
      <button onClick={() => navigate({ to: "/carrinho" })} aria-label="Voltar ao carrinho" className="flex size-9 items-center justify-center rounded-full text-[#071342]"><ArrowLeft className="size-[19px]"/></button>
      <div className="ml-2 flex flex-1 items-center gap-2.5"><div className="flex size-8 items-center justify-center rounded-lg bg-[#071342] text-sm font-black text-white">V</div><div><p className="text-[14px] font-black leading-none tracking-tight">Van<span className="text-[#f4a900]">Mox</span></p><p className="mt-0.5 text-[9px] font-medium uppercase tracking-[0.12em] text-slate-400">Pagamento seguro</p></div></div>
      <LockKeyhole className="size-[17px] text-slate-400"/>
    </header>

    <main className="px-4 pt-5">
      <section className="border-b border-slate-100 pb-5"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Área de pagamento</p><h1 className="mt-1 text-[22px] font-extrabold tracking-tight">Finalize sua reserva</h1><p className="mt-1 text-xs leading-relaxed text-slate-500">Confira o total e escolha a forma de pagamento.</p></section>

      {temSelecao ? <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">Resumo da viagem</p><p className="mt-1 truncate text-[14px] font-bold">{reserva.origem} <span className="px-1 text-slate-300">→</span> {reserva.destino}</p><p className="mt-1 text-[11px] text-slate-500">{reserva.horario} · {reserva.passageiros} passageiro{reserva.passageiros === 1 ? "" : "s"}</p></div><div className="shrink-0 text-right"><p className="text-[10px] font-medium text-slate-400">Total</p><p className="mt-0.5 text-[23px] font-black tracking-tight text-[#071342]">{brl(valor)}</p></div></div></section> : <section className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-center"><p className="text-sm font-bold">Nenhuma viagem selecionada</p><p className="mt-1 text-xs leading-relaxed text-slate-500">Escolha uma viagem no carrinho para que o total apareça e o pagamento seja liberado.</p><button onClick={() => navigate({ to: "/carrinho" })} className="press mt-4 h-10 rounded-lg bg-[#071342] px-5 text-xs font-bold text-white">Voltar ao carrinho</button></section>}

      <section className="mt-6"><div className="flex items-center justify-between"><h2 className="text-[15px] font-extrabold">Forma de pagamento</h2><div className="flex items-center gap-2 opacity-90"><PixLogo className="size-5"/><CardBrands/></div></div><p className="mt-1 text-[11px] text-slate-500">Selecione uma opção para continuar.</p>
        <div className="mt-3 space-y-2">
          <button onClick={() => { setMetodo("pix"); setErro(null); }} className={optionClass(metodo === "pix")}><PixLogo className="size-7 shrink-0"/><div className="min-w-0 flex-1"><p className="text-[13px] font-bold">Pix</p><p className="mt-0.5 text-[10px] text-slate-500">Aprovação imediata</p></div>{metodo === "pix" ? <span className="flex size-5 items-center justify-center rounded-full bg-[#159b86] text-[11px] font-bold text-white">✓</span> : <ChevronRight className="size-4 text-slate-400"/>}</button>
          ...
          <button onClick={() => { setMetodo("debito"); setErro(null); }} className={optionClass(metodo === "debito")}><CardBrands/><div className="min-w-0 flex-1"><p className="text-[13px] font-bold">Cartão de débito</p><p className="mt-0.5 text-[10px] text-slate-500">Visa ou Mastercard</p></div>{metodo === "debito" ? <span className="flex size-5 items-center justify-center rounded-full bg-[#071342] text-[11px] font-bold text-white">✓</span> : <ChevronRight className="size-4 text-slate-400"/>}</button>
        </div>
      </section>

      {metodo !== "pix" && <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4"><div className="mb-3 flex items-center justify-between"><h3 className="text-[13px] font-bold">Dados do cartão</h3><CardBrands/></div><div className="space-y-2.5"><input inputMode="numeric" autoComplete="cc-number" placeholder="Número do cartão" value={cartao.numero} onChange={e => setCartao(c => ({ ...c, numero: e.target.value.replace(/\D/g, "").slice(0, 19) }))} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-[13px] outline-none focus:border-[#071342] focus:bg-white"/><input autoComplete="cc-name" placeholder="Nome impresso no cartão" value={cartao.titular} onChange={e => setCartao(c => ({ ...c, titular: e.target.value }))} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-[13px] outline-none focus:border-[#071342] focus:bg-white"/><div className="flex gap-2.5"><input inputMode="numeric" autoComplete="cc-exp" placeholder="MM/AA" value={cartao.validade} onChange={e => setCartao(c => ({ ...c, validade: e.target.value.replace(/\D/g, "").slice(0, 4).replace(/^(\d{2})(\d)/, "$1/$2") }))} className="w-1/2 rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-[13px] outline-none focus:border-[#071342] focus:bg-white"/><input inputMode="numeric" autoComplete="cc-csc" placeholder="CVV" value={cartao.cvv} onChange={e => setCartao(c => ({ ...c, cvv: e.target.value.replace(/\D/g, "").slice(0, 4) }))} className="w-1/2 rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-[13px] outline-none focus:border-[#071342] focus:bg-white"/></div>{metodo === "credito" && <select value={cartao.parcelas} onChange={e => setCartao(c => ({ ...c, parcelas: Number(e.target.value) }))} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-[13px] outline-none focus:border-[#071342]"><option value={1}>1x de {brl(valor)}</option>{Array.from({ length: 11 }, (_, i) => i + 2).map(n => <option key={n} value={n}>{n}x de {brl(valor / n)}</option>)}</select>}</div></section>}

      {metodo === "pix" && temSelecao && <div className="mt-3 text-[10px] leading-relaxed text-slate-500">O código Pix será gerado após tocar em <strong className="text-[#071342]">Pagar agora</strong>.</div>}
      ...
      <button onClick={pagar} disabled={carregando || !temSelecao} className="press mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#071342] text-[14px] font-bold text-white transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-40">{carregando ? <Loader2 className="size-5 animate-spin"/> : temSelecao ? `Pagar ${brl(valor)}` : "Selecione uma viagem para pagar"}</button>
